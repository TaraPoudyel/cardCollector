...
module.exports.run =  async (bot, msg, arg, con) => {
  if (msg.author.id == '0') return msg.channel.send("https://cdn.discordapp.com/attachments/812774382379597834/844374366052810752/risitas.png")
  ...
  if(!isNaN(arg[1]) && arg[1] <= 1001) {generatedRarity = arg[1]}
  if(!isNaN(arg[2]) && arg[2] <= max && arg[2] > 0) {spawnCard = arg[2]}
  let rarityNum;
  ...
  if(arg[3] == "max") {highest(rarityNum)}
  ...
  con.query(`SELECT sapphire FROM players WHERE user_id = '${msg.author.id}'`, (err, rows) => {
    ...
    if(sapphire < 0 ) return msg.channel.send("Not enough Sapphire")
    else {
  ...
  async function sendImg(imgSrc,char_name, series_id, series_name) {
    const frameSrc = './images/testbg1.png'
    const canvas = Canvas.createCanvas(250, 420)
    const ctx = canvas.getContext('2d')

    const charimg = await Canvas.loadImage(imgSrc)
  //  const background = await Canvas.loadImage(frameSrc)
  ...
  function highest(rarityNum) {
    luck = (rarityNum*10) + (10 * rarityNum)
    hp = (rarityNum*100) + (50 * rarityNum)
    speed = (rarityNum*10) + (10 * rarityNum)
    strength = (rarityNum*10) + (10 * rarityNum)
    totalStats = luck + hp + speed + strength;
    averageStats = (totalStats - baseStats[rarityNum - 1]) / offset[rarityNum - 1] * 100
  }
